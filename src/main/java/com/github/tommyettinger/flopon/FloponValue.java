 ...
 * for (FloponValue entry : map)
 * 	System.out.println(entry.name + " = " + entry.asString());
 * // No allocation:
 * for (FloponValue entry = map.child; entry != null; entry = entry.next)
 * 	System.out.println(entry.name + " = " + entry.asString());
 * </pre>
 * 
 * @author Nathan Sweet */
public class FloponValue implements Iterable<FloponValue> {
	...
	public String name;
	/** May be null. */
	public FloponValue child, parent;
	/** May be null. When changing this field the parent {@link #size()} may need to be changed. */
	public FloponValue next, prev;
	public int size;

	public FloponValue(ValueType type) {
		this.type = type;
	}

	/** @param value May be null. */
	public FloponValue(@Null String value) {
		set(value);
	}

	public FloponValue(double value) {
		set(value, null);
	}

	public FloponValue(long value) {
		set(value, null);
	}

	public FloponValue(double value, String stringValue) {
		set(value, stringValue);
	}

	public FloponValue(long value, String stringValue) {
		set(value, stringValue);
	}

	public FloponValue(boolean value) {
		set(value);
	}

	/** Returns the child at the specified index. This requires walking the linked list to the specified entry, see
	 * {@link FloponValue} for how to iterate efficiently.
	 * @return May be null. */
	public @Null FloponValue get (int index) {
		FloponValue current = child;
	 ...
	 * @return May be null. */
	public @Null FloponValue get (String name) {
		FloponValue current = child;
	...
	public FloponIterator iterator (String name) {
		FloponValue current = get(name);
		if (current == null) {
			FloponIterator iter = new FloponIterator();
			iter.entry = null;
			return iter;
		}
	...
	public FloponValue require (int index) {
		FloponValue current = get(index);
	...
	/** Returns the child with the specified name.
	 * @throws IllegalArgumentException if the child was not found. */
	public FloponValue require (String name) {
		FloponValue current = get(name);
	 ...
	 * {@link FloponValue} for how to iterate efficiently.
	 * @return May be null. */
	public @Null FloponValue remove (int index) {
		FloponValue child = get(index);
	 ...
	 * @return May be null. */
	public @Null FloponValue remove (String name) {
		FloponValue child = get(name);
		if (child == null) return null;
		if (child.prev == null) {
			this.child = child.next;
			if (this.child != null) this.child.prev = null;
	...
	public String[] asStringArray () {
		...
		int i = 0;
		for (FloponValue value = child; value != null; value = value.next, i++) {
			String v;
			switch (value.type) {
			case stringValue:
				v = value.stringValue;
	 ...
	 * @return May be null. */
	public @Null FloponValue getChild (String name) {
		FloponValue child = get(name);
	...
	public void addChild (FloponValue value) {
	...
	public String toFlopon (OutputType outputType) {
	...
	private void json (FloponValue object, StringBuilder buffer, OutputType outputType) {
		if (object.isObject()) {
			if (object.child == null)
				...
				while (true) {
					buffer.append('{');
					int i = 0;
					for (FloponValue child = object.child; child != null; child = child.next) {
	...
	public FloponIterator iterator () {
		return new FloponIterator();
	}
	...
	public String trace () {
		...
		if (parent.type == ValueType.array) {
			trace = "[]";
			int i = 0;
			for (FloponValue child = parent.child; child != null; child = child.next, i++) {
				if (child == this) {
					trace = "[" + i + "]";
					break;
				}
	...
	private void prettyPrint (FloponValue object, StringBuilder buffer, int indent, PrettyPrintSettings settings) {
	...
	private void prettyPrint (FloponValue object, Writer writer, int indent, PrettyPrintSettings settings) throws IOException {
		OutputType outputType = settings.outputType;
		if (object.isObject()) {
			if (object.child == null)
				writer.append("{}");
	...
	static private boolean isFlat (FloponValue object) {
		for (FloponValue child = object.child; child != null; child = child.next)
			if (child.isObject() || child.isArray()) return false;
		return true;
	}

	static private boolean isNumeric (FloponValue object) {
		for (FloponValue child = object.child; child != null; child = child.next)
	...
	public class FloponIterator implements Iterator<FloponValue>, Iterable<FloponValue> {
		FloponValue entry = child;
		FloponValue current;
		...
		public Iterator<FloponValue> iterator () {
			return this;
		}
	}
