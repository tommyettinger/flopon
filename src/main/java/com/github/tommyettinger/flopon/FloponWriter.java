 ...
 * @author Nathan Sweet */
public class FloponWriter extends Writer {
	final Writer writer;
	private final Array<FloponObject> stack = new Array();
	private FloponObject current;
	private boolean named;
	private OutputType outputType = OutputType.json;
	private boolean quoteLongValues = false;

	public FloponWriter(Writer writer) {
		this.writer = writer;
	}
	...
	public FloponWriter name (String name) throws IOException {
	...
	public FloponWriter object () throws IOException {
		requireCommaOrName();
		stack.add(current = new FloponObject(false));
		return this;
	}

	public FloponWriter array () throws IOException {
		requireCommaOrName();
		stack.add(current = new FloponObject(true));
		return this;
	}

	public FloponWriter value (Object value) throws IOException {
		if (quoteLongValues
	...
	public FloponWriter json (String json) throws IOException {
		requireCommaOrName();
		writer.write(json);
		return this;
	}
	...
	public FloponWriter object (String name) throws IOException {
		return name(name).object();
	}

	public FloponWriter array (String name) throws IOException {
		return name(name).array();
	}

	public FloponWriter set (String name, Object value) throws IOException {
		return name(name).value(value);
	}

	/** Writes the specified JSON value, without quoting or escaping. */
	public FloponWriter json (String name, String json) throws IOException {
		return name(name).json(json);
	}

	public FloponWriter pop () throws IOException {
	...
	private class FloponObject {
		final boolean array;
		boolean needsComma;

		FloponObject(boolean array) throws IOException {
			this.array = array;
			writer.write(array ? '[' : '{');
		}
