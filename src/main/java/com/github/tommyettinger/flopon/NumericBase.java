 ...
 * encoding data for a URI (typically meant for the Internet). New here is {@link #BASE86}, which is also the largest
 ...
 * this could be useful for obfuscating plain-text saved data so the average player can't read it. There's also the
 * specialized base-64 version, {@link #FLOPON_SAFE}, meant to enable numbers encoded in that base to be written to
 * flopon files without quoting.
 * <br>
...
public class NumericBase {
	 ...
	 */
	public static final NumericBase BASE2 = new NumericBase("01");
	/**
	 * Octal, using the digits 0-7.
	 */
	public static final NumericBase BASE8 = new NumericBase("01234567");
	/**
	 * Decimal, using the digits 0-9.
	 */
	public static final NumericBase BASE10 = new NumericBase("0123456789");
	/**
	 * Hexadecimal, using the digits 0-9 and then A-F (case-insensitive).
	 */
	public static final NumericBase BASE16 = new NumericBase("0123456789ABCDEF");
	/**
	 ...
	 */
	private static final List<NumericBase> BASES = Arrays.asList(BASE2, BASE8, BASE10, BASE16, BASE36, BASE64, URI_SAFE, FLOPON_SAFE, BASE86);
	 ...
	 * <br>
	 * This should not be changed after the NumericBase has been used; changing this makes a NumericBase incompatible with its
	 ...
	 * creating a different NumericBase, but if you are doing that to obfuscate output, you can use
	...
	/**
	 * Constructs a NumericBase with the given digits, ordered from smallest to largest, with any letters in the digits treated
	 ...
	 */
	public NumericBase(String digits) {
		this(digits, true, ' ', '+', '-');
	}
	 ...
	 */
	public NumericBase(String digits, boolean caseInsensitive, char padding, char positiveSign, char negativeSign) {
		paddingChar = padding;
	...
	/**
	 * Returns a seemingly-gibberish NumericBase that uses a radix of 72 and a randomly-ordered set of characters to represent
	 * the different digit values. This is randomized by a {@link Random}, so if the parameter is seeded identically
	 * (and is the same implementation), then an equivalent NumericBase will be produced. This randomly chooses 72 digits from
	 ...
	 *
	 * @param random a Random (or libGDX RandomXS128, preferably) used to shuffle the possible digits
	 * @return a new NumericBase with 72 random digits, as well as a random positive and negative sign
	 */
	public static NumericBase scrambledBase (Random random) {
		...
		NumericBase base = new NumericBase("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789~!@#$%^&*-", false, ' ', plus, minus);
	...
	/**
	 * Stores this NumericBase as a compact String; the String this produces is usually given to
	 * {@link #deserializeFromString(String)} to restore the NumericBase. Note that if you are using
	 * {@link #scrambledBase(Random)}, you are also able to serialize the EnhancedRandom or its state, and that
	 * can be used to produce a scrambled base again; this could be useful to conceal a scrambled base slightly.
	 *
	 * @return a String that can be given to {@link #deserializeFromString(String)} to obtain this NumericBase again
	 */
	...
	/**
	 * Given a String of a serialized NumericBase (almost always produced by {@link #serializeToString()}), this re-creates
	 * that NumericBase and returns it.
	 *
	 * @param data a String that was almost always produced by {@link #serializeToString()}
	 * @return the NumericBase that {@code data} stores
	 */
	public static NumericBase deserializeFromString (String data) {
		int len;
		if ((len = data.length()) >= 5) {
			return new NumericBase(data.substring(0, len - 4), data.charAt(len - 4) != '0', data.charAt(len - 3), data.charAt(len - 2), data.charAt(len - 1));
		}
		throw new IllegalArgumentException("The given data does not store a serialized NumericBase.");
	}

	/**
	 * Converts the given {@code number} to this NumericBase as unsigned, returning a new String.
	...
	/**
	 * Converts the given {@code number} to this NumericBase as unsigned, appending the result to
	 * {@code builder}.
	 *
	...
	/**
	 * Converts the given {@code number} to this NumericBase as signed, returning a new String.
	...
	/**
	 * Converts the given {@code number} to this NumericBase as signed, appending the result to
	 ...
	 * early, this behaves as you would expect for a number with fewer digits, and simply doesn't fill the larger places.
	 *
	 * @param cs    a char array containing only the digits in this NumericBase and/or an optional initial sign (usually + or -)
	 * @param start the (inclusive) first character position in cs to read
	 * @param end   the (exclusive) last character position in cs to read (this after reading enough chars to represent the largest possible value)
	 * @return the long that cs represents
	 */
	...
	/**
	 * Given a String containing numbers in this NumericBase, separated by instances of delimiter, returns those numbers as a
	...
	/**
	 * Given a String containing numbers in this NumericBase, separated by instances of delimiter, returns those numbers as an
	 ...
	 * from elements, in this NumericBase, separated by delimiter.
	 *
	 * @param delimiter the separator to put between numbers
	 * @param elements  a long array; if null, this returns an empty String
	 * @return a String containing all numbers in elements, written in this NumericBase, separated by delimiter
	 */
	 ...
	 * the StringBuilder all longs from elements, in this NumericBase, separated by delimiter.
	 *
	 ...
	 * the StringBuilder all ints from elements, in this NumericBase, separated by delimiter.
	 *
	 * @param sb        the StringBuilder to append to; if null, this returns null
	 * @param delimiter the separator to put between numbers
	 ...
	 * the StringBuilder all shorts from elements, in this NumericBase, separated by delimiter.
	 *
	 * @param sb        the StringBuilder to append to; if null, this returns null
	 * @param delimiter the separator to put between numbers
	 ...
	 * the StringBuilder all bytes from elements, in this NumericBase, separated by delimiter.
	 *
	 * @param sb        the StringBuilder to append to; if null, this returns null
	 * @param delimiter the separator to put between numbers
	 ...
	 * (as numbers) from elements, in this NumericBase, separated by delimiter.
	 *
	 * @param delimiter the separator to put between numbers
	 ...
	 * the StringBuilder all chars (as numbers) from elements, in this NumericBase, separated by delimiter.
	 *
	 * @param sb        the StringBuilder to append to; if null, this returns null
	 * @param delimiter the separator to put between numbers
	 ...
	 * from elements, in this NumericBase, separated by delimiter. This uses {@link #appendSigned(StringBuilder, double)},
	 ...
	 * the StringBuilder all doubles from elements, in this NumericBase, separated by delimiter. This uses
	 ...
	 * from elements, in this NumericBase, separated by delimiter. This uses {@link #appendSigned(StringBuilder, float)},
	 * which means this does not produce human-readable numbers.
	 *
	 * @param delimiter the separator to put between numbers
	 ...
	 * the StringBuilder all floats from elements, in this NumericBase, separated by delimiter. This uses
	 ...
	 * here, so it can be in NumericBase.
	 *
	...
	public boolean equals (Object o) {
		...
		NumericBase base = (NumericBase)o;
