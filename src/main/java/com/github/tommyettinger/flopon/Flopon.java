...
import com.github.tommyettinger.flopon.FloponValue.PrettyPrintSettings;
import com.github.tommyettinger.flopon.FloponWriter.OutputType;
...
public class Flopon {
	...
	private FloponWriter.OutputType outputType;
	...
	public Flopon(OutputType outputType) {
	 ...
	 * @see FloponWriter#setOutputType(OutputType) */
	public void setOutputType (OutputType outputType) {
		this.outputType = outputType;
	}

	/** Default is false.
	 * @see FloponWriter#setQuoteLongValues(boolean) */
	...
	private OrderedMap<String, FieldMetadata> getFields (Class type) {
		...
		sortFields(type, nameToField.orderedKeys());
		typeToFields.put(type, nameToField);
		return nameToField;
	}

	public String toFlopon (@Null Object object) {
		return toFlopon(object, object == null ? null : object.getClass(), (Class)null);
	}

	public String toFlopon (@Null Object object, @Null Class knownType) {
		return toFlopon(object, knownType, (Class)null);
	}

	/** @param knownType May be null if the type is unknown.
	 * @param elementType May be null if the type is unknown. */
	public String toFlopon (@Null Object object, @Null Class knownType, @Null Class elementType) {
		StringWriter buffer = new StringWriter();
		toFlopon(object, knownType, elementType, buffer);
		return buffer.toString();
	}

	public void toFlopon (@Null Object object, FileHandle file) {
		toFlopon(object, object == null ? null : object.getClass(), null, file);
	}

	/** @param knownType May be null if the type is unknown. */
	public void toFlopon (@Null Object object, @Null Class knownType, FileHandle file) {
		toFlopon(object, knownType, null, file);
	}

	/** @param knownType May be null if the type is unknown.
	 * @param elementType May be null if the type is unknown. */
	public void toFlopon (@Null Object object, @Null Class knownType, @Null Class elementType, FileHandle file) {
		Writer writer = null;
		try {
			writer = file.writer(false, "UTF-8");
			toFlopon(object, knownType, elementType, writer);
		} catch (Exception ex) {
	...
	public void toFlopon (@Null Object object, Writer writer) {
		toFlopon(object, object == null ? null : object.getClass(), null, writer);
	}

	/** @param knownType May be null if the type is unknown. */
	public void toFlopon (@Null Object object, @Null Class knownType, Writer writer) {
		toFlopon(object, knownType, null, writer);
	}

	/** @param knownType May be null if the type is unknown.
	 * @param elementType May be null if the type is unknown. */
	public void toFlopon (@Null Object object, @Null Class knownType, @Null Class elementType, Writer writer) {
		setWriter(writer);
		try {
	...
	/** Sets the writer where JSON output will be written. This is only necessary when not using the toFlopon methods. */
	public void setWriter (Writer writer) {
		if (!(writer instanceof FloponWriter)) writer = new FloponWriter(writer);
		this.writer = (FloponWriter)writer;
		this.writer.setOutputType(outputType);
		this.writer.setQuoteLongValues(quoteLongValues);
	}

	public FloponWriter getWriter () {
		return writer;
	}
	 ...
	 * @return May be null. */
	public @Null <T> T fromFlopon (Class<T> type, Reader reader) {
		return readValue(type, null, new FloponReader().parse(reader));
	}

	/** @param type May be null if the type is unknown.
	 * @param elementType May be null if the type is unknown.
	 * @return May be null. */
	public @Null <T> T fromFlopon (Class<T> type, Class elementType, Reader reader) {
		return readValue(type, elementType, new FloponReader().parse(reader));
	}

	/** @param type May be null if the type is unknown.
	 * @return May be null. */
	public @Null <T> T fromFlopon (Class<T> type, InputStream input) {
		return readValue(type, null, new FloponReader().parse(input));
	}

	/** @param type May be null if the type is unknown.
	 * @param elementType May be null if the type is unknown.
	 * @return May be null. */
	public @Null <T> T fromFlopon (Class<T> type, Class elementType, InputStream input) {
		return readValue(type, elementType, new FloponReader().parse(input));
	}

	/** @param type May be null if the type is unknown.
	 * @return May be null. */
	public @Null <T> T fromFlopon (Class<T> type, FileHandle file) {
		try {
			return readValue(type, null, new FloponReader().parse(file));
		} catch (Exception ex) {
	 ...
	 * @param elementType May be null if the type is unknown.
	 * @return May be null. */
	public @Null <T> T fromFlopon (Class<T> type, Class elementType, FileHandle file) {
		try {
			return readValue(type, elementType, new FloponReader().parse(file));
		} catch (Exception ex) {
			throw new SerializationException("Error reading file: " + file, ex);
		}
	}
	...
	/** @param type May be null if the type is unknown.
	 * @return May be null. */
	public @Null <T> T fromFlopon (Class<T> type, char[] data, int offset, int length) {
		return readValue(type, null, new FloponReader().parse(data, offset, length));
	}

	/** @param type May be null if the type is unknown.
	 * @param elementType May be null if the type is unknown.
	 * @return May be null. */
	public @Null <T> T fromFlopon (Class<T> type, Class elementType, char[] data, int offset, int length) {
		return readValue(type, elementType, new FloponReader().parse(data, offset, length));
	}

	/** @param type May be null if the type is unknown.
	 * @return May be null. */
	public @Null <T> T fromFlopon (Class<T> type, String json) {
		return readValue(type, null, new FloponReader().parse(json));
	}

	/** @param type May be null if the type is unknown.
	 * @return May be null. */
	public @Null <T> T fromFlopon (Class<T> type, Class elementType, String json) {
		return readValue(type, elementType, new FloponReader().parse(json));
	}

	public void readField (Object object, String name, FloponValue jsonData) {
		readField(object, name, name, null, jsonData);
	}

	public void readField (Object object, String name, @Null Class elementType, FloponValue jsonData) {
		readField(object, name, name, elementType, jsonData);
	}

	public void readField (Object object, String fieldName, String jsonName, FloponValue jsonData) {
		readField(object, fieldName, jsonName, null, jsonData);
	}

	/** @param elementType May be null if the type is unknown. */
	public void readField (Object object, String fieldName, String jsonName, @Null Class elementType, FloponValue jsonMap) {
		Class type = object.getClass();
		FieldMetadata metadata = getFields(type).get(fieldName);
		if (metadata == null) throw new SerializationException("Field not found: " + fieldName + " (" + type.getName() + ")");
		Field field = metadata.field;
	 ...
	 * @param elementType May be null if the type is unknown. */
	public void readField (@Null Object object, Field field, String jsonName, @Null Class elementType, FloponValue jsonMap) {
		FloponValue floponValue = jsonMap.get(jsonName);
		if (floponValue == null) return;
		try {
			field.set(object, readValue(field.getType(), elementType, floponValue));
		} catch (ReflectionException ex) {
			...
			SerializationException ex = new SerializationException(runtimeEx);
			ex.addTrace(floponValue.trace());
			ex.addTrace(field.getName() + " (" + field.getDeclaringClass().getName() + ")");
			throw ex;
		}
	}

	public void readFields (Object object, FloponValue jsonMap) {
		Class type = object.getClass();
		OrderedMap<String, FieldMetadata> fields = getFields(type);
		for (FloponValue child = jsonMap.child; child != null; child = child.next) {
	...
	/** Called for each unknown field name encountered by {@link #readFields(Object, FloponValue)} when {@link #ignoreUnknownFields}
	 ...
	 *         {@link #readFields(Object, FloponValue)}. */
	...
	/** @param type May be null if the type is unknown.
	 * @return May be null. */
	public @Null <T> T readValue (String name, @Null Class<T> type, FloponValue jsonMap) {
		return readValue(type, null, jsonMap.get(name));
	}

	/** @param type May be null if the type is unknown.
	 * @return May be null. */
	public @Null <T> T readValue (String name, @Null Class<T> type, T defaultValue, FloponValue jsonMap) {
		FloponValue floponValue = jsonMap.get(name);
		if (floponValue == null) return defaultValue;
		return readValue(type, null, floponValue);
	}

	/** @param type May be null if the type is unknown.
	 * @param elementType May be null if the type is unknown.
	 * @return May be null. */
	public @Null <T> T readValue (String name, @Null Class<T> type, @Null Class elementType, FloponValue jsonMap) {
	 ...
	 * @param elementType May be null if the type is unknown.
	 * @return May be null. */
	public @Null <T> T readValue (String name, @Null Class<T> type, @Null Class elementType, T defaultValue, FloponValue jsonMap) {
		FloponValue floponValue = jsonMap.get(name);
		return readValue(type, elementType, defaultValue, floponValue);
	}

	/** @param type May be null if the type is unknown.
	 * @param elementType May be null if the type is unknown.
	 * @return May be null. */
	public @Null <T> T readValue (@Null Class<T> type, @Null Class elementType, T defaultValue, FloponValue jsonData) {
	...
	/** @param type May be null if the type is unknown.
	 * @return May be null. */
	public @Null <T> T readValue (@Null Class<T> type, FloponValue jsonData) {
	 ...
	 * @param elementType May be null if the type is unknown.
	 * @return May be null. */
	public @Null <T> T readValue (@Null Class<T> type, @Null Class elementType, FloponValue jsonData) {
		...
		if (jsonData.isObject()) {
			...
			if (typeName != null && ClassReflection.isAssignableFrom(Collection.class, type)) {
				...
				if (object instanceof ObjectSet) {
					...
					for (FloponValue child = jsonData.getChild("values"); child != null; child = child.next)
		...
		if (jsonData.isNumber()) {
			...
			jsonData = new FloponValue(jsonData.asString());
		}
	...
	public void copyFields (Object from, Object to) {
		...
		for (Entry<String, FieldMetadata> entry : getFields(from.getClass())) {
	...
	public String prettyPrint (@Null Object object, int singleLineColumns) {
		return prettyPrint(toFlopon(object), singleLineColumns);
	}

	public String prettyPrint (String json, int singleLineColumns) {
		return new FloponReader().parse(json).prettyPrint(outputType, singleLineColumns);
	}

	public String prettyPrint (@Null Object object, PrettyPrintSettings settings) {
		return prettyPrint(toFlopon(object), settings);
	}

	public String prettyPrint (String json, PrettyPrintSettings settings) {
		return new FloponReader().parse(json).prettyPrint(settings);
	}
	...
	static public interface Serializer<T> {
		public void write(Flopon flopon, T object, Class knownType);

		public T read(Flopon flopon, FloponValue jsonData, Class type);
	}
	...
	static abstract public class ReadOnlySerializer<T> implements Serializer<T> {
		...
		abstract public T read (Flopon flopon, FloponValue jsonData, Class type);
	}

	static public interface Serializable {
		public void write(Flopon flopon);

		public void read(Flopon flopon, FloponValue jsonData);
	}
}
